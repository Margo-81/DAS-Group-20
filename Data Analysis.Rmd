...
fit1<-glm(time_at_shelter~intake_type+outcome_type+chip_status,data=dataset20,family=poisson(link = "log")) #build poisson glm model
...
fit3<-glm(time_at_shelter~intake_type+outcome_type+chip_status,data=dataset20,family=quasipoisson(link = "log")) #build quasipoisson glm model
...
plot(x=pred,y=stand.resid,xlab="Predicted count",ylab="Standardised Pearson residuals",main="Regular likelihood",ylim=c(-2,4))
abline(h=c(-1.5,0,4),lty="dotted",col="red")
pred<-predict(fit3,type="response")
stand.resid<-rstandard(model=fit3,type="pearson")
plot(x=pred,y=stand.resid,xlab="Predicted count",ylab="Standardised Pearson residuals",main="Quasi-likelihood",ylim=c(-2,4))
abline(h=c(-1.5,0,4),lty="dotted",col="red")
```
